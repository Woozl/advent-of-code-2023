...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sum: isize = lines.fold(0, |acc, line| acc + get_next_val(line.collect()));
...
fn get_next_val(level: Vec<isize>) -> isize {
    ...
    return get_next_val(next_level) + level.last().unwrap();
}
