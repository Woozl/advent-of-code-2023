...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let value_map: HashMap<char, usize> =
        cards.iter().enumerate().map(|(i, c)| (*c, i + 2)).collect();
    ...
    let mut scored_hands = hands
        .map(|(hand, bid)| {
            ...
            for (i, c) in hand.chars().enumerate() {
                ...
                // keep track of the jokers and don't add them to the frequency map
                if c == 'J' {
                    jokers += 1;
                } else {
            ...
            let mut freq_distrib: Vec<usize> = freq_map.values().cloned().collect();
            freq_distrib.sort();

            let freq_distrib_slice = freq_distrib.as_mut_slice();

            let hand_classification: usize = if jokers == 5 {
                // if jokers == 5, that means we have a five-of-a-kind
                // this case kind of breaks the way the other matching code
                // works because freq_distrib_slice will have a length of zero.
                7
            } else {
                // if we have jokers, add them to the largest value in the frequency map
                // to get the highest possible hand classification
                *freq_distrib_slice.last_mut().unwrap() += jokers;
                match freq_distrib_slice {
                    [5] => 7,             // Five of a kind
                    ...
                    d => {
                        panic!("Found a card distribution that doesn't fix a classification {d:?}")
                    }
                }
            };
